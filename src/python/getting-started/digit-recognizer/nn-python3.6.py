...
def trainRF():
    ...
    storeModel(rfClf, os.path.join(data_dir, 'output/Result_sklearn_nn.model'))
...
def preRF():
    startTime = time.time()
    # 加载模型和数据
    clf=getModel(os.path.join(data_dir, 'output/Result_sklearn_nn.model'))
    print('model:',clf)
    ...
    # 结果的输出
    saveResult(result,os.path.join(data_dir, 'output/Result_sklearn_nn.csv'))
    print("finish!")
    stopTime = time.time()
